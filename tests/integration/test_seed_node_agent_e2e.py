...
def _join_node(
    *,
    ...
    node_id: str,
    ordinal: int,
) -> tuple[NodeMembershipSession, SeedHTTPClient, dict[str, Any], dict[str, Any]]:
    ...
    session = NodeMembershipSession(
        ...
        swarm_id="swarm-local-e2e",
        seed_node_id="seed-node",
        signer=load_or_create_node_signer(root / node_id / "identity" / "node.key"),
    ...
    return session, client, bundle, request
    ...
    try:
        with SeedHTTPServer(coordinator, host="127.0.0.1", port=0) as seed_server:
            ...
            for ordinal, node_id in enumerate(NODE_IDS, start=1):
                session, client, bundle, request = _join_node(
            ...
            second = _NodeClient(
                ...
                deployment_id=graph.deployment_id,
                artifact_root=tmp_path,
                socket_root=socket_root / "b",
            )
            node_processes = {NODE_IDS[0]: first, NODE_IDS[1]: second}
            ...
            for node_id in NODE_IDS:
                snapshot = node_processes[node_id].command("snapshot")["observation"][
                    "details"
                ]
