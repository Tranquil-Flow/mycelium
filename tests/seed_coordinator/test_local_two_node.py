...
def _native_sidecar_pid(service_pid: int) -> int:
    ...
    assert Path(executable).name == SIDECAR_BINARY.name
    return sidecar_pid
...
def test_seed_two_memberships_assign_and_run_native_iroh_inference(
    ...
    try:
        ...
        with SeedHTTPServer(coordinator, host="127.0.0.1", port=0) as seed_server:
            ...
            current_qualification = qualification_authority.current()
            route_ready = (
                current_qualification is not None
                and current_qualification.route_ready
            )
            assert route_ready is False
    finally:
