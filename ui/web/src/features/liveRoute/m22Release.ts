...
export type M22ReleaseEvidence = Readonly<{
  ...
  services: Readonly<{ package_count: number; roles: readonly string[]; platform_classes: readonly string[]; continuous_renewal: boolean; bounded_restart: boolean; foreground_route_restart_verified: boolean; restart_verified: boolean; coordinator_restart_verified: boolean; log_rotation: boolean; graceful_drain: boolean }>;
...
export function decodeM22Release(value: unknown): M22ReleaseEvidence {
  ...
  const services = exact(root.services, ['package_count','roles','platform_classes','continuous_renewal','bounded_restart','foreground_route_restart_verified','restart_verified','coordinator_restart_verified','log_rotation','graceful_drain'], 'm22 services');
