...
export type M22ReleaseEvidence = Readonly<{
  ...
  reviewer: Readonly<{ bundle_version: string; bundle_manifest_digest: string; preflight_idempotent: boolean; surrogate_verified: boolean; external_network: boolean; assigned_stage: boolean; inference_completed: boolean; negative_case_verified: boolean }>;
...
export function decodeM22Release(value: unknown): M22ReleaseEvidence {
  ...
  const reviewer = exact(root.reviewer, ['bundle_version','bundle_manifest_digest','preflight_idempotent','surrogate_verified','external_network','assigned_stage','inference_completed','negative_case_verified'], 'm22 reviewer');
