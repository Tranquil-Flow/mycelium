...
def _activation_bytes_for_assignments(
    ...
    dtypes = {assignment.get("runtime", {}).get("dtype") for assignment in assignments}
    if len(dtypes) != 1:
...
def build_replicated_execution_graph(
    assignments_by_placement: Mapping[str, dict[str, Any]],
    proofs_by_placement: Mapping[str, Mapping[str, Any]],
    route_plan: Mapping[str, Any],
    *,
    link_scheme: str = "iroh",
    runtime_scheme: str = "iroh",
) -> ExecutionGraph:
    """Bind a Planner-v2 replica graph without flattening its legal tracks."""

    if (
        route_plan.get("protocol") != "mycelium.route_plan.v2"
        or not assignments_by_placement
        or set(assignments_by_placement) != set(proofs_by_placement)
    ):
        raise PhysicalDeploymentError("replicated_execution_graph_inputs_invalid")
    placement_intent = {
        item.get("placement_id"): item
        for item in route_plan.get("placements", [])
        if isinstance(item, Mapping) and isinstance(item.get("placement_id"), str)
    }
    if set(placement_intent) != set(assignments_by_placement):
        raise PhysicalDeploymentError("replicated_execution_graph_placement_mismatch")
    node_ids = [
        assignment["node_id"] for assignment in assignments_by_placement.values()
    ]
    if len(node_ids) != len(set(node_ids)):
        raise PhysicalDeploymentError("physical_replica_node_hosts_multiple_placements")
    normalized_proofs = {
        placement_id: json.loads(canonical_json(proof))
        for placement_id, proof in proofs_by_placement.items()
    }
    groups: dict[str, list[Mapping[str, Any]]] = {}
    for item in placement_intent.values():
        groups.setdefault(item["replica_group_id"], []).append(item)
    ordered_groups = sorted(
        groups.items(),
        key=lambda pair: (
            pair[1][0]["layer_range"]["start"],
            pair[0],
        ),
    )
    stages: list[Stage] = []
    for group_index, (group_id, members) in enumerate(ordered_groups):
        ranges = {
            (item["layer_range"]["start"], item["layer_range"]["end"])
            for item in members
        }
        if (
            len(ranges) != 1
            or sum(item.get("primary") is True for item in members) != 1
        ):
            raise PhysicalDeploymentError("replicated_execution_graph_group_invalid")
        start, end = next(iter(ranges))
        placements: list[Placement] = []
        component_roles: tuple[str, ...] | None = None
        for intent in sorted(members, key=lambda item: item["placement_id"]):
            placement_id = intent["placement_id"]
            assignment = assignments_by_placement[placement_id]
            if (
                assignment["node_id"] != intent["node_id"]
                or assignment["range"]["start_layer"] != start
                or assignment["range"]["end_layer_exclusive"] != end
            ):
                raise PhysicalDeploymentError(
                    "replicated_execution_graph_assignment_mismatch"
                )
            roles = tuple(assignment["components"])
            if component_roles is None:
                component_roles = roles
            elif component_roles != roles:
                raise PhysicalDeploymentError(
                    "replicated_execution_graph_component_mismatch"
                )
            placements.append(
                Placement(
                    ...
                    assignment_id=assignment["assignment_id"],
                    stage_signature="pending-stage-signature",
                    load_proof_digest=layer_load_proof_digest(
                        normalized_proofs[placement_id]
                    ),
                    runtime_backend=assignment["runtime"]["backend"],
                    runtime_endpoint=(
                        f"{runtime_scheme}://{assignment['node_id']}/"
        ...
        stages.append(
            Stage(
                stage_id=f"replica-stage-{group_index:03d}",
                layer_range=LayerRange(
                    start_layer=start,
                    end_layer_exclusive=end,
                    layer_count=end - start,
                ),
                component_roles=component_roles or ("decoder",),
                stage_cost=StageCost(
                    prefill_work_units_per_prompt_token=1.0,
                    decode_work_units_per_token=1.0,
                    kv_bytes_per_context_token=32,
    ...
    first = next(iter(assignments_by_placement.values()))

    def edge_from_plan(item: Mapping[str, Any], *, prefix: str) -> PlacementEdge:
        src = item["src_placement_id"]
        dst = item["dst_placement_id"]
        return PlacementEdge(
            edge_id=f"{prefix}:{src}->{dst}",
            from_placement_id=src,
            to_placement_id=dst,
            link_id=(
                f"{link_scheme}:{assignments_by_placement[src]['node_id']}"
                f"->{assignments_by_placement[dst]['node_id']}"
            ),
        )

    loopback_edges = tuple(
        edge_from_plan(item, prefix="loopback")
        for item in route_plan.get("loopbacks", [])
    )
    if len(stages) == 1 and not loopback_edges:
        loopback_edges = tuple(
            PlacementEdge(
                edge_id=f"loopback:{placement.placement_id}->{placement.placement_id}",
                from_placement_id=placement.placement_id,
                to_placement_id=placement.placement_id,
                link_id=f"local:{placement.node_id}",
            )
            for placement in stages[0].placements
        )
    graph = ExecutionGraph(
        deployment_id=first["deployment_id"],
        deployment_epoch=first["deployment_epoch"],
        topology_version=1,
        model_id=first["model_id"],
        resolved_commit=first["resolved_commit"],
        manifest_digest=first["manifest_digest"],
        entry_stage_id=stages[0].stage_id,
        final_stage_id=stages[-1].stage_id,
        hidden_size=first["runtime"]["model_config"]["n_embd"],
        activation_bytes=_activation_bytes_for_assignments(
            tuple(assignments_by_placement.values())
        ),
        token_envelope_bytes=9,
        stages=tuple(stages),
        edges=tuple(
            edge_from_plan(item, prefix="forward")
            for item in route_plan.get("forward_edges", [])
        ),
        loopback_edges=loopback_edges,
    )
    signed_stages = tuple(
        replace(
            stage,
            placements=tuple(
                replace(
                    ...
                    stage_signature=_stage_signature(
                        graph,
                        stage,
                        normalized_proofs[placement.placement_id],
                    ),
                )
    ...
    try:
        return validate_execution_graph(replace(graph, stages=signed_stages))
    except (TypeError, ValueError) as exc:
        raise PhysicalDeploymentError("replicated_execution_graph_invalid") from exc
...
def build_physical_device_states(graph: ExecutionGraph) -> dict[str, DeviceState]:
    ...
    return {
        node_id: DeviceState(
            ...
            neighbor_rtt_ms={
                neighbor: 0.0 if neighbor == node_id else 1.0 for neighbor in node_ids
            },
...
class PhysicalDeployment:
    ...
    def evidence_document(self) -> dict[str, Any]:
        ...
        document = {
            ...
            "reference_assignment": assignment_evidence(self.reference_assignment),
...
def _validate_runtime_dtype(runtime_dtype: str) -> str:
    if (
        not isinstance(runtime_dtype, str)
        or runtime_dtype not in _PREPARABLE_RUNTIME_DTYPES
    ):
...
def _load_checkpoint_index(
    ...
    checkpoint_index = {
        ...
        "weight_map": {
            tensor_name: _MONOLITHIC_WEIGHT_FILE for tensor_name in header.tensor_names
        },
    }
...
def _materialize_local_model_source(
    ...
    for asset in source.tokenizer_assets:
        if asset in reserved_paths:
            raise _invalid_local_model_source(
                "tokenizer_asset_conflicts_with_model_file"
            )
        relative = _safe_local_relative_path(asset, field="tokenizer_asset")
...
class _IdentityAwareLocalFetcher:
    ...
    def __call__(
        ...
        try:
            ...
            raise PhysicalDeploymentError(
                "local_model_fetch_cache_root_missing"
            ) from exc
