...
_TAG_BYTES = 32
_MAX_RECORD_BYTES = _HEADER.size + OPERATIONAL_MAX_FRAME_BYTES + _TAG_BYTES
...
class SidecarClient:
    ...
    def connect(self) -> None:
        with self._lock:
            ...
            try:
                ...
                _write_prefixed(stream, _canonical_json(hello))
                encoded_server = _read_prefixed(stream, _MAX_HELLO_BYTES)
    ...
    def configure_peer(self, endpoint_id: str, endpoint_addr: dict[str, Any]) -> None:
        ...
        payload = _canonical_json(
            {"endpoint_id": endpoint_id, "endpoint_addr": endpoint_addr}
        )
        message_id = os.urandom(16)
        self._expect_ack(*self._request(_CONFIGURE_PEER, message_id, payload), message_id)
    ...
    def cancel(self, message_id: bytes) -> None:
        _validate_message_id(message_id)
        self._expect_ack(*self._request(_CANCEL, message_id, b""), message_id)
        ...
        *,
        ...
        with self._lock:
            ...
            try:
                ...
                _write_prefixed(stream, encoded)
                self._send_sequence = _increment_sequence(self._send_sequence)
                response = _read_prefixed(stream, _MAX_RECORD_BYTES)
...
def _encode_record(
    ...
    _validate_message_id(message_id)
    if len(payload) > OPERATIONAL_MAX_FRAME_BYTES:
...
def _decode_record(
    ...
    if kind not in {_SEND, _RECEIVE, _DELIVERY, _ACK, _CANCEL, _CONFIGURE_PEER, _PING, _ERROR}:
...
def _write_prefixed(stream: socket.socket, payload: bytes) -> None:
    if not payload or len(payload) > _MAX_RECORD_BYTES:
        raise ProtocolError("invalid_length")
    stream.sendall(struct.pack(">I", len(payload)) + payload)


def _read_prefixed(stream: socket.socket, maximum: int) -> bytes:
