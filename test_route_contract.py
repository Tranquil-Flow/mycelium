...
class RouteContractTests(unittest.TestCase):
   def v1(self):
      return {
         ...
         "model": {"model_id": "org/model", "num_layers": 5},
         "route": [
   ...
   def test_upgrade_converts_inclusive_ranges_exactly_once(self):
      ...
      upgraded = rc.upgrade_route_plan_v1(original)

      self.assertEqual(original, before)
      self.assertEqual(upgraded["protocol"], "mycelium.route_plan.v2")
   ...
   def test_one_layer_and_full_model_ranges_validate(self):
      ...
      rc.validate_route_plan_v2(one)

      full = copy.deepcopy(one)
      full["model"] = {"model_id": "full", "num_layers": 5}
      full["route"][0]["range"] = {
         "start_layer": 0,
         "end_layer_exclusive": 5,
         "layer_count": 5,
      }
      rc.validate_route_plan_v2(full)
   ...
   def test_gap_fails_closed(self):
      ...
      with self.assertRaisesRegex(ValueError, "gap"):
         rc.upgrade_route_plan_v1(broken)
