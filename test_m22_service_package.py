...
def test_service_package_is_deterministic_bounded_and_shell_free() -> None:
    first = build_service_package(config())
    second = build_service_package(config())
    assert first == second
    plist = plistlib.loads(first["_payloads"]["org.mycelium.node-a.plist"])
    assert plist["ProgramArguments"] == config()["argv"]
...
def test_written_package_is_owner_only_and_manifest_matches(tmp_path: Path) -> None:
    result = write_service_package(config(), tmp_path / "package")
    paths = tuple((tmp_path / "package").iterdir())
    assert len(paths) == 4
    assert all(stat.S_IMODE(path.stat().st_mode) == 0o600 for path in paths)
    manifest = json.loads((tmp_path / "package" / "service-package-manifest.json").read_text())
