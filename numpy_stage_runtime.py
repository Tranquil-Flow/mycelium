"""Assignment-bound, local-only NumPy GPT-2 stage loading and load-proof emission.

Mirrors the MLX loader contract (*runtime_loader.py*) exactly, but uses
lazy-imported NumPy so that importing core modules never triggers a NumPy
dependency. The backend identity is ``numpy`` and must not be confused with
an MLX load proof.
"""
...
_NUMPY_LAYER_LOAD_PROOF_PROTOCOL = "mycelium.layer_load_proof.v1"
...
_NUMPY_RUNTIME_DTYPES = {
    "float16": None,  # resolved lazily
    "float32": None,  # resolved lazily
}


def _lazy_numpy() -> Any:
    """Lazy-import NumPy so core modules do not depend on it."""
    import numpy as np

    if _NUMPY_RUNTIME_DTYPES["float16"] is None:
        _NUMPY_RUNTIME_DTYPES["float16"] = np.float16
        _NUMPY_RUNTIME_DTYPES["float32"] = np.float32
    return np
...
class LoadedNumpyStage:
    """Materialized assignment tensors plus deterministic local proof evidence."""

    tensors: Mapping[str, Any]  # MappingProxyType of np.ndarray
    resolved_aliases: Mapping[str, Any]
    probe_output: Any  # np.ndarray
...
def _validate_runtime(runtime: Any) -> tuple[dict[str, Any], Any]:
    ...
    runtime_dtype = _NUMPY_RUNTIME_DTYPES[normalized["dtype"]]
    return normalized, runtime_dtype
...
def _validate_component_ownership(
    ...
    if "lm_head" in component_keys and "lm_head" not in aliases and component_keys["lm_head"] != ["lm_head.weight"]:
        raise _fail(
...
def _validate_safetensors_header(
    ...
    """Return one strictly validated Safetensors header for subsequent loading."""
    try:
...
def _load_exact_tensors(
    ...
    """Load exactly the assigned tensors from verified Safetensors files using NumPy."""
    np = _lazy_numpy()
    ...
    for assigned_record in files:
        ...
        try:
            ...
            header_length, header = _validate_safetensors_header(handle, path)
            ...
            for key in selected:
                ...
                start_off, end_off = entry["data_offsets"]

                if dtype_str not in _SUPPORTED_SOURCE_DTYPES:
                    raise _fail(
                        f"unverified or quantized source dtype for tensor {key}: {dtype_str}"
                    )

                # Read the raw bytes
                data_offset = 8 + header_length + start_off
                byte_length = end_off - start_off
                handle.seek(data_offset)
                raw_bytes = handle.read(byte_length)
                if len(raw_bytes) != byte_length:
                    raise _fail(f"truncated tensor data for {key}: {path}")

                # Convert to NumPy array
                np_dtype_map = {"F32": np.float32, "F16": np.float16}
                np_source = np.frombuffer(raw_bytes, dtype=np_dtype_map[dtype_str])
                np_source = np_source.reshape(shape)
                loaded[key] = np_source.astype(runtime_dtype, copy=True)

            fingerprint_after = _artifact_fingerprint(os.fstat(handle.fileno()))
...
def _layer_norm(
    hidden: Any, weight: Any, bias: Any, epsilon: float
) -> Any:
    np = _lazy_numpy()
    compute = hidden.astype(np.float32)
    mean = np.mean(compute, axis=-1, keepdims=True)
    variance = np.mean(np.square(compute - mean), axis=-1, keepdims=True)
    normalized = (compute - mean) / np.sqrt(variance + epsilon)
    return (normalized * weight.astype(np.float32) + bias.astype(np.float32)).astype(
        hidden.dtype
    )


def _gelu_new(value: Any) -> Any:
    np = _lazy_numpy()
...
def _softmax(value: Any, axis: int) -> Any:
    np = _lazy_numpy()
    dtype = value.dtype
    compute = value.astype(np.float32)
...
def _run_gpt2_probe(
    ...
    if "input_embedding" in components:
        ...
        ).astype(_NUMPY_RUNTIME_DTYPES[runtime["dtype"]])
    ...
    if "lm_head" in components:
        head_key = aliases.get("lm_head", {}).get("tensor_keys", ["lm_head.weight"])[0]
        hidden = np.matmul(hidden, tensors[head_key].T)
...
def execute_loaded_stage(
    loaded_stage: LoadedNumpyStage,
    *,
    token_ids: Any | None = None,
    hidden_states: Any | None = None,
) -> Any:
    """Execute exactly the GPT-2 components bound by one ``LoadedNumpyStage``.

    Runtime identity, layer range, roles, and aliases come from the immutable
    load proof. Entry stages accept rank-two integer token IDs; all other stages
    accept rank-three hidden states.
    """
    np = _lazy_numpy()
    ...
    if not isinstance(loaded_stage, LoadedNumpyStage):
    ...
    expected_dtype = _NUMPY_RUNTIME_DTYPES[runtime["dtype"]]
    ...
    if has_embedding:
        ...
        if np.any(token_ids < 0) or np.any(token_ids >= config["vocab_size"]):
            reject("token_bounds_exceeded")
        positions = np.arange(sequence, dtype=np.int64)
        hidden = (
            tensors[f"{namespace}wte.weight"][token_ids.astype(np.int64)]
        ...
        hidden = hidden_states.astype(expected_dtype, copy=True)
    ...
    if "lm_head" in components:
        ...
        if (
            ...
            reject("invalid_loaded_stage_aliases")
        hidden = np.matmul(hidden, tensors[head_keys[0]].T)
    if not np.isfinite(hidden).all():
        reject("nonfinite_stage_output")
    result = np.ascontiguousarray(hidden)
...
def load_assignment_stage(
    ...
    *,
    ...
    try:
        ...
        # Make probe_output immutable
        probe_output.flags.writeable = False
        proof = {
            "protocol": _NUMPY_LAYER_LOAD_PROOF_PROTOCOL,
...
class NumpyStageBackend:
    """Protocol adapter exposing the assignment-bound NumPy stage executor."""
